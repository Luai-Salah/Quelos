...
namespace Quelos
{
    struct ConsolePanal
    {
        ...
        static char* Strdup(const char* s) { IM_ASSERT(s); size_t len = strlen(s) + 1; void* buf = malloc(len); IM_ASSERT(buf); return static_cast<char*>(memcpy(buf, (const void*)s, len)); }
        ...
        {
            ConsolePanal* console = static_cast<ConsolePanal*>(data->UserData);
