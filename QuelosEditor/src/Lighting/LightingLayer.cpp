...
namespace Lighting
{
        ...
        : m_EditorCamera(eCam)
    {
        ...
        m_ViewPosition = UniformBuffer::Create(sizeof(glm::vec3), 2);
    ...
    void Lighting::OnUpdate(TimeStep ts)
    {
        ...
        m_ViewPosition->SetData(glm::value_ptr(glm::vec3{ m_EditorCamera.GetPosition() }), sizeof(glm::vec3));

        m_Light.Position = glm::vec4(m_LightPosition, 1.0f);

        m_Light.Ambient = glm::vec4{ 0.2f };
        m_Light.Diffuse = glm::vec4{ 0.5f };
        ...
        {
            m_Transform = glm::translate(glm::mat4(1.0f), cubePositions[i]) * glm::rotate(glm::mat4(1.0f), 20.0f * i, {1.0f, 0.3f, 0.5f});
