...
namespace Quelos
{
	namespace Utils
	{
		...
		static GLenum ShaderTypeFromString(const std::string& type)
		{
			...
			QS_CORE_ASSERT(false, "Unknown shader type!");
			return 0;
		}
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
#if QS_SHADER_DEBUG
			QS_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
#endif // QS_SHADER_DEBUG
		}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			QS_CORE_ERROR("Could not open file '{0}'", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			QS_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
			std::string type = source.substr(begin, eol - begin);
			QS_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
			QS_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
...
#if QS_SHADER_DEBUG
		QS_CORE_TRACE("OpenGLShader::Reflect - {0} {1}\n\t{2} uniform buffers\n\t{3} resources",
			Utils::GLShaderStageToString(stage), m_FilePath, resources.uniform_buffers.size(),
...
#if QS_SHADER_DEBUG
			QS_CORE_TRACE("\t{0}\n\t\tSize = {1}\n\t\tBinding = {2}\n\t\tMembers = {3}",
				resource.name, bufferSize, binding, memberCount);
#endif // QS_SHADER_DEBUG
		}
	}
