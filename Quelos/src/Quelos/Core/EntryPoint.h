...
{
	QS_PROFILE_BEGIN_SESSION("Startup", "Logs\\Profiling\\QS_Profile-Startup.json");
	auto app = Quelos::CreateApplication(Quelos::ApplicationCommandLineArgs { argc, argv });
	QS_PROFILE_END_SESSION();

	QS_PROFILE_BEGIN_SESSION("Runtime", "Logs\\Profiling\\QS_Profile-Runtime.json");
	app->Run();
	QS_PROFILE_END_SESSION();

	QS_PROFILE_BEGIN_SESSION("Shutdown", "Logs\\Profiling\\QS_Profile-Shutdown.json");
	delete app;
	QS_PROFILE_END_SESSION();
